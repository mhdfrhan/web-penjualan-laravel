...
@section('container')
    <section>
                ...
                <div class="relative h-full w-full flex flex-col justify-center max-w-xl animate__animated animate__zoomIn animate__faster">
                    <div class="relative bg-white rounded-lg shadow">
                        <div class="flex items-start justify-between p-4 border-b rounded-t ">
                            <h3 class="text-xl font-semibold text-gray-900">Tambah pembeli</h3><button type="button"
                        ...
                        <div class="p-6 space-y-6">
                            <form action="{{ route('pembeli.tambah') }}" method="POST" enctype="multipart/form-data"> @csrf
                                <div class="grid md:grid-cols-2 gap-6 mb-6">
                                    ...
                                    <div>
                                        <div class="relative"> <input type="email" id="email" name="email"
                                    ...
                                    <div>
                                        <div class="relative"> <input type="password" id="password_confirmation"
                                ...
                                <div>
                                    <div class="relative">
                                        <textarea rows="4" id="alamat" name="alamat"
                                            ...
                                            autocomplete="off" placeholder=" " required value="{{ old('alamat') }}">{{ old('alamat') }}</textarea> <label for="alamat"
                                ...
                                <div class="mt-6"> <img class="img-preview w-40" alt=""> <input class=""
                                ...
                                <div class="flex items-center justify-end gap-4 mt-4"><button type="button"
                                        ...
                                        type="submit"
                                        class="px-4 py-2.5 bg-gradient-to-b from-violet-500 to-violet-600 text-white rounded-lg shadow-lg shadow-violet-600/30 mt-4 hover:scale-105 duration-500">Tambah</button>
                                </div>
                            </form>
                        </div>
                    </div>
        ...
        </div>@include('dashboard.partials.message') <div class="relative overflow-x-auto">
            <table class="table">
                <thead class="">
                    <tr>
                        <th scope="col">No</th>
                ...
                <tbody>
                    @foreach ($pembeli as $i => $p)
                        <tr>
                            <th scope="row">{{ $pembeli->firstItem() + $i }}</th>
                            <td scope="row">
                                @if ($p->image)
                                    <a class="cursor-pointer" data-fancybox
                                        data-src="{{ asset('img/user/' . $p->image) }}"> <img
                                            src="{{ asset('img/user/' . $p->image) }}"
                                            class="inline w-24 rounded-lg h-auto" alt="gambar{{ $p->name }}"> </a>
                                @else
                                    <a class="cursor-pointer" data-fancybox data-src="/img/no-photo.png"> <img
                                            src="/img/no-photo.png" class="w-24 rounded-lg h-auto"
                                            alt="gambar{{ $p->name }}"> </a>
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="flex items-center gap-x-2">
                                    <div> <button class="viewBtn"> <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                    ...
                                    <div>
                                        <form action="{{ route('user.edit', $p->slug) }}" method="GET"> @csrf <button
                                                class="editBtn"> <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                    class="w-5 h-5">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <div>
                                        <form action="/dashboard/user/hapus/" method="POST" id="formHapus"> @csrf
                                            <button type="submit" data-modal-target="hapusModal"
                                                data-modal-toggle="hapusModal" class="hapusBtn"
                                                value="{{ $p->slug }}" data-name="{{ $p->name }}"><svg
        ...
        @if (!empty($pembeli->links()))
            <div class="mt-8 space-x-3">{{ $pembeli->links('vendor.pagination.tailwind') }}</div>
            @endif @if (count($pembeli))
            @else<div class="p-4 mt-4 text-sm text-red-800 rounded-lg bg-red-50 text-center" role="alert"><span
                        class="font-medium">Tidak ada data pembeli!</span></div>
            @endif
            {{-- hapus modal --}}
